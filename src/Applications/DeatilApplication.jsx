...
useEffect(()=>{
  ...
  const response=await axios.get(`https://intern-area-backned.onrender.com/api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://intern-area-backned.onrender.com/api/application/${id}`,{action})
